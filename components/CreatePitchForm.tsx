// components/CreatePitchForm.tsx
'use client';
...
export default function CreatePitchForm({ 
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-xl w-full space-y-4">
      <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 gap-y-1">
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-300">Target Funding Goal (USDC / Stable)</label>
          <input
            type="number"
            required
            min="1"
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-300">Target Network Node</label>
      ...
      <div className="space-y-1">
        <label className="text-xs font-medium text-slate-300">Venture Thesis & Milestone Architecture</label>
        <textarea
          required
          placeholder="Detail your value proposition, engineering timeline, and structural milestone budget divisions..."
          value={description}
      ...
      <button
        ...
        className="w-full py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 disabled:opacity-50 text-white font-bold text-sm rounded-xl transition-all shadow-md shadow-cyan-950/30"
      >
